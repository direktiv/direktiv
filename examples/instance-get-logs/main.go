...
//	A get instance requests is then sent to grpc and the returned instance details are printed.
func main() {
	var err error
	...
	// send grpc request
	log.Printf("Getting logs from '%s'", instanceID)
	...
	for _, logEntry := range resp.WorkflowInstanceLogs {
		log.Printf(*logEntry.Message)
	}
	log.Printf("- End of logs")
