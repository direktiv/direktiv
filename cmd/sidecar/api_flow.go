...
var (
	actionIDCtxKey        = &ContextKey{"ActionID"}
	containerURLCtxKey    = &ContextKey{"URL"}
	actionCtxKey          = &ContextKey{"Action"}
	filesLocationCtxKey   = &ContextKey{"FilesLocation"}
	maxResponseSizeCtxKey = &ContextKey{"MaxResponseSizeCtxKey"}
	outRequestCtxKey      = &ContextKey{"OutRequest"}
)
...
func setupAPIForFlow(config Config, actionCtl *sync.Map) *chi.Mux {
	router := chi.NewRouter()
	router.Use(middleware.Logger)
	router.Use(middleware.Recoverer)
	// Router for handling external requests.
	router.Post("/", func(w http.ResponseWriter, r *http.Request) {
		executeFunction(r, w, config.UserServiceURL, actionCtl, ActionBuilder{})
	})
	return router
}

func executeFunction(r *http.Request, w http.ResponseWriter, userServiceURL string, actionCtl *sync.Map, actionDeserialize ActionDeserialize) {
	...
	// 4. Prepare Request
	req, err := prepareRequestToUserContainer(actionID, userServiceURL, filesLocation, ctl.UserInput)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	// 5. Execute Request
	resp, err := executeRequestToUserContainer(ctx, req)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func prepareRequestToUserContainer(actionID, userServiceURL, filesLocation string, userInput []byte) (*http.Request, error) {

	// 1. Construct the base HTTP request
	req, err := http.NewRequest(http.MethodPost, userServiceURL+"?action_id="+actionID, nil)
	if err != nil {
...
func executeRequestToUserContainer(ctx context.Context, req *http.Request) (*http.Response, error) {
	...
	maxResponseSize, ok := ctx.Value(maxResponseSizeCtxKey).(int)
	if ok && resp.ContentLength > int64(maxResponseSize) {
