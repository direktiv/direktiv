...
docker-start: ## Create a local docker deployment.
	...
	sed -i '' 's|^ *-.*:/etc/nginx/conf.d/default.conf|    - ./nginx.template:/etc/nginx/conf.d/default.conf|' ./docker-compose.yaml
...
docker-headless: ## Create a local docker deployment without an included UI container.
	sed -i '' 's|^ *-.*:/etc/nginx/conf.d/default.conf|    - ./nginx.headless.template:/etc/nginx/conf.d/default.conf|' ./docker-compose.yaml
