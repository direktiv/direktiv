...
const NewPolicy = ({
  ...
  const { mutate: createFile, isPending } = useCreateFile({
    onSuccess: (data) => {
      if (namespace) {
        navigate({
          to: "/n/$namespace/explorer/policy/$",
          from: "/n/$namespace",
  ...
  const onSubmit: SubmitHandler<FormInput> = ({ name, fileContent }) => {
    createFile({
      ...
      payload: {
        name,
        data: encode(JSON.stringify(fileContent)),
        type: "policy",
  ...
  const formId = `new-policy-${path}`;
  return (
    <>
      <DialogHeader>
        <DialogTitle>
          <Shield /> {t("pages.explorer.tree.newPolicy.title")}
        </DialogTitle>
      </DialogHeader>
        ...
        <form id={formId} onSubmit={handleSubmit(onSubmit)}>
          <fieldset className="flex items-center gap-5">
            <label className="w-[90px] text-right text-[14px]" htmlFor="name">
              {t("pages.explorer.tree.newPolicy.nameLabel")}
            </label>
            <Input
              id="name"
              placeholder={t("pages.explorer.tree.newPolicy.namePlaceholder")}
              {...register("name")}
            />
          </fieldset>
        </form>
      ...
      <DialogFooter>
        <DialogClose asChild>
          <Button variant="ghost">
            {t("pages.explorer.tree.newPolicy.cancelBtn")}
          </Button>
        </DialogClose>
        <Button
          type="submit"
        ...
        >
          ...
          {t("pages.explorer.tree.newPolicy.createBtn")}
        </Button>
      </DialogFooter>
    </>
  );
