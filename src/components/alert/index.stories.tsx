...
const TemplateState: ComponentStory<typeof Alert> = (args) => {
  const [errorMsg, setErrorMsg] = useState(
    "fetching workflows" as null | string
  );
  return (
    <>
      {errorMsg ? (
        ...
        <Button
          color="error"
          onClick={() => {
            setErrorMsg("fetching workflows");
          }}
        >
          Show Alert
        </Button>
...
Error.story = {
  parameters: {
    docs: {
      description: {
        story:
          "An error alert example with the onClose prop being used to allow the user to hide the alert.",
      },
    },
  },
};
