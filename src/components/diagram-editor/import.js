...
export function importFromWorkflowData(diagramEditor, setFunctions, wfData) {
  ...
  for (let i = 0; i < wfData.states.length; i++) {
    ...
    const result = ActionsNodes.filter(
      (ActionsNode) =>
        ActionsNode.family === "primitive" && ActionsNode.type === state.type
    );
...
function importDefaultProcessTransformCallback(state, transformKey) {
  ...
  if (!oldTransform) {
    ...
    oldTransform.trim().startsWith(`js(`) &&
