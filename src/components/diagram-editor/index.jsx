...
import {
  ...
  nodeGetInputConnections,
  onSubmitCallbackMap,
  onValidateSubmitCallbackMap,
  setConnections,
  sortNodes,
  unescapeJSStrings,
} from "./util";
              ...
              >
                ...
                {ActionsNodes[index].info.link ? (
                  <a
                    ...
                    href={`${ActionsNodes[index].info.link}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <VscInfo />
...
function FunctionsList(props) {
  const { functionList, setFunctionList, namespace, functionDrawerWidth } =
    props;
  ...
  const namespaceServiceHook = useNamespaceServices(
    Config.url,
    false,
    namespace,
    apiKey
  ...
  const namespaceNodesHook = useNodes(
    Config.url,
    false,
    namespace,
    "/",
  ...
  if (
    namespaceServiceHook.data === null ||
  ...
  const ajv = new Ajv();
  const functionSchemas = GenerateFunctionSchemaWithEnum(
    namespaceServiceHook.data.map((a) => a.serviceName),
    globalServiceHook.data.map((a) => a.serviceName),
    namespaceNodesHook.data
  );
  ...
  if (
    ...
    formData &&
    formData.type === "knative-namespace"
  ) {
        ...
        actionButtons={[
          {
            ...
            onClick: async () => {
              ...
              if (!validate(formData)) {
                if (
                  formData.type === "knative-namespace" &&
                  namespaceServiceHook.data.length === 0
                ) {
                  throw Error("Invalid Function: No Namespace Services Exist");
                } else if (
                  formData.type === "knative-global" &&
                  globalServiceHook.data.length === 0
                ) {
              ...
              const result = functionList.filter(
                (functionItem) => functionItem.id === formData.id
              );
              if (result.length > 0) {
...
export default function DiagramEditor(props) {
  ...
  const [actionDrawerWidthOld, setActionDrawerWidthOld] =
    useState(MaxDrawerSize);
  ...
  useEffect(() => {
    if (selectedNode) {
      ...
      if (getSchema) {
        setSelectedNodeSchema(
          getSchema(selectedNode.data.schemaKey, functionList)
        );
      } else {
    ...
    editor.on("connectionCreated", function (ev) {
      ...
      if (outputIsPrimitive) {
        if (ev.output_class === errorOutput && inNode.name !== "CatchError") {
          ...
          ev.output_class !== errorOutput &&
          inNode.name === "CatchError"
        ) {
      ...
      if (isInvalidConnection) {
        ...
        if (outNode.outputs[ev.output_class].connections.length > 1) {
          const removeConnection =
            outNode.outputs[ev.output_class].connections[0];
          editor.removeSingleConnection(
            ev.output_id,
  ...
  useEffect(() => {
    if (diagramEditor && wf) {
      if (load) {
        setUnhandledData(
          importFromWorkflowData(diagramEditor, setFunctionList, wf)
        );
      }
      setLoad(false);
    }
          ...
          <input
            ...
            onKeyDown={(ev) => {
              if (ev.key === "Enter" && contextMenuResults.length > 0) {
                const newNode = contextMenuResults[0].item
                  ? contextMenuResults[0].item
                  : contextMenuResults[0];
                setBlock(true);
                CreateNode(
                  diagramEditor,
                  newNode,
                  contextMenuAnchorPoint.x,
                  contextMenuAnchorPoint.y
                );
        ...
        <div className="toolbar">
          <div
            ...
            onClick={() => {
              ...
              if (nonInitNodes.length > 0) {
                setError(
                  `Failed - Node not initialized: ${nonInitNodes.join(", ")}`
                );
                return;
              }
              ...
              for (const nodeID in rawData) {
                ...
                if (
                  !rawNode.compiled &&
                  rawNode.data.family === "primitive" &&
                  !nodeGetInputConnections(rawNode)
                ) {
              ...
              if (updateWorkflow) {
                ...
                const workflowStr = unescapeJSStrings(
                  PrettyYAML.stringify(wfData)
                );
          ...
          >
            {actionDrawerMinWidth === 0 ? (
              <>
                <VscListUnordered
                  style={{ fontSize: "256px", width: "48px" }}
                />
                <div>Show Nodes</div>
              </>
            ) : (
        ...
        <FlexBox style={{ overflow: "hidden" }}>
          ...
          >
            <Resizable
              style={{
                ...resizeStyle,
                pointerEvents: actionDrawerWidth === 0 ? "none" : "",
                visibility: actionDrawerWidth === 0 ? "hidden" : "visible",
              }}
            ...
            <Resizable
              style={{
                ...resizeStyle,
                pointerEvents: functionDrawerWidth === 0 ? "none" : "",
                visibility: functionDrawerWidth === 0 ? "hidden" : "visible",
              }}
            ...
            <div
              ...
              onContextMenu={(ev) => {
                ...
                const parentIsNode =
                  ev.target.offsetParent.className.includes("drawflow-node");
                const targetIsNode =
                  ev.target.className.includes("drawflow-node");
            ...
            <ModalHeadless
              ...
              title={`Node Details: ${
                selectedNode ? selectedNode.data.id : ""
              }`}
              actionButtons={[
                {
                  label: "Submit",
                  ...
                  onClick: () => {
                    ...
                    diagramEditor.updateNodeDataFromId(
                      updatedNode.id,
                      updatedNode.data
                    );
                    ...
                    setNodeInitTracker((old) => {
                      old[selectedNode.id] = {
                        init: updatedNode.data.init,
                        stateID: updatedNode.data.id,
                      };
                      return {
                        ...old,
                      };
