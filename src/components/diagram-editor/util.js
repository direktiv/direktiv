...
export function insertHangingNodes(
  nodeID,
  previousNodeID,
  previousState,
  rawData,
...
function processTransform(stateData, transformKey) {
  ...
  const keyValue = stateData[transformKey]["keyValue"]
    ? stateData[transformKey]["keyValue"]
    : {};
  const jqQuery = stateData[transformKey]["jqQuery"]
    ? stateData[transformKey]["jqQuery"]
    : "";
  const rawYAML = stateData[transformKey]["rawYAML"]
    ? stateData[transformKey]["rawYAML"]
    : "";
...
function validateFormTransform(formTransformData) {
  ...
  if (
    formTransformData.selectionType &&
    formTransformData.selectionType === "YAML" &&
    formTransformData.rawYAML !== ""
  ) {
    try {
...
export function CreateNode(diagramEditor, node, clientX, clientY, rawXY) {
  ...
  if (!rawXY) {
    posX =
      clientX *
        (diagramEditor.precanvas.clientWidth /
          (diagramEditor.precanvas.clientWidth * diagramEditor.zoom)) -
      diagramEditor.precanvas.getBoundingClientRect().x *
        (diagramEditor.precanvas.clientWidth /
          (diagramEditor.precanvas.clientWidth * diagramEditor.zoom));
    posY =
      clientY *
        (diagramEditor.precanvas.clientHeight /
          (diagramEditor.precanvas.clientHeight * diagramEditor.zoom)) -
      diagramEditor.precanvas.getBoundingClientRect().y *
        (diagramEditor.precanvas.clientHeight /
          (diagramEditor.precanvas.clientHeight * diagramEditor.zoom));
  }
  ...
  return diagramEditor.addNode(
    node.name,
    node.connections.input,
    node.connections.output,
    posX,
    posY,
    `node ${node.family} type-${node.type}`,
    {
      family: node.family,
      type: node.type,
      init: !node.info.requiresInit,
      ...node.data,
    },
    newNodeHTML,
    false
