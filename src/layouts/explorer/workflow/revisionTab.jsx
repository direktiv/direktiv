            ...
            <ContentPanelBody style={{ padding: "0px" }}>
              {tabBtn === 0 ? (
                <FlexBox col style={{ overflow: "hidden" }}>
                  ...
                  <FlexBox
                    className="gap editor-footer"
                    style={{ borderTop: "1px solid white", overflow: "hidden" }}
                  >
              ...
              {tabBtn === 1 ? (
                <WorkflowDiagram disabled={true} workflow={workflow} />
              ) : (
                ""
              )}
              {tabBtn === 2 ? (
...
function RevertTrafficAmount(props) {
  ...
  const TrafficAmount = useCallback(() => {
    ...
    const sliderClass =
      routeIndex === 0
        ? "traffic-mini-distribution"
        : "traffic-mini2-distribution";

    return (
      <FlexBox className="col revision-label-tuple">
        <div>Traffic amount</div>
        <div style={{ width: "100%" }}>
          <Slider
            defaultValue={routes[routeIndex].weight}
            className={sliderClass}
            disabled={true}
          />
          <div>{`${routes[routeIndex].weight}%`}</div>
        </div>
      </FlexBox>
    );
  }, [routes, revisionName]);
...
function TagRevisionBtn(props) {
  ...
  return (
    <Modal
      ...
      requiredFields={[{ tip: "tag is required", value: tag }]}
    >
      <FlexBox>
        <input
          autoFocus
        ...
        <ContentPanelBody style={{ flexDirection: "column" }}>
                ...
                <FlexBox style={{ alignItems: "center", marginTop: "10px" }}>
                  ...
                  >
                    ...
                    {revisions.map((obj) => {
                      ...
                      return (
                        <option key={GenerateRandomKey()} value={obj.name}>
                          {obj.name}
                        </option>
                      );
                    })}
          ...
          >
            ...
            <Button
              ...
              tooltip="Run workflow with router traffic"
            >
              Run
            </Button>
          </FlexBox>
