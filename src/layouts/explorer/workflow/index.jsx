    ...
    <>
      ...
      >
        <TabBar
          setRouter={setRouter}
          router={router}
          getWorkflowRouter={getWorkflowRouter}
        ...
        <FlexBox col gap>
          {activeTab === 0 ? (
            <OverviewTab
              getSuccessFailedMetrics={getSuccessFailedMetrics}
              router={router}
              namespace={namespace}
          ...
          {activeTab === 1 ? (
            <>
              <RevisionSelectorTab
                ...
                setRouter={setRouter}
                editWorkflowRouter={editWorkflowRouter}
                getWorkflowRouter={getWorkflowRouter}
                ...
                getWorkflowRevisionData={getWorkflowRevisionData}
                searchParams={searchParams}
                setSearchParams={setSearchParams}
          ...
          {activeTab === 3 ? (
            <SettingsTab
              ...
              workflowData={data}
              setWorkflowLogToEvent={setWorkflowLogToEvent}
              namespace={namespace}
              workflow={filepath}
            />
          ) : null}
      ...
      <FlexBox col>
        <FlexBox
          ...
          style={{
            ...
            alignItems: "center",
            borderBottom: "1px solid #536470",
            padding: "6px 10px 6px 10px",
          }}
        >
          <div style={{ paddingRight: "6px" }}>Problem</div>
          <div
            style={{
              display: "flex",
          ...
          >
            <pre style={{ margin: "0px", fontSize: "medium" }}>
              {errors.length}
            </pre>
          </div>
        </FlexBox>
        <FlexBox
          col
          style={{ padding: "6px 10px 6px 10px", overflowY: "scroll" }}
        >
          {errors.length > 0 ? (
            <>
              {errors.map((err, i) => (
...
function WorkingRevision(props) {
  ...
  const [tabBtn, setTabBtn] = useState(
    searchParams.get("revtab") !== null
      ? parseInt(searchParams.get("revtab"))
      : 0
  );
  ...
  const pushOpLoadingState = useCallback(
    (target, value) => {
      ...
      old["IsLoading"] =
        opLoadingStates["Save"] ||
        opLoadingStates["Update"] ||
        opLoadingStates["Undo"];
    ...
    <FlexBox style={{ width: "100%" }}>
      <ContentPanel style={{ width: "100%" }}>
        ...
        <ContentPanelBody style={{ padding: "0px" }}>
            ...
            <FlexBox col style={{ overflow: "hidden" }}>
              <FlexBox>
                <DirektivEditor
                  ...
                  value={workflow}
                  dvalue={oldWf}
                  setDValue={setWorkflow}
                />
              </FlexBox>
              <FlexBox className="gap editor-footer">
                <WorkingRevisionErrorBar
                  errors={errors}
                    ...
                    onOpen={() => {
                      try {
                        ...
                        if (
                          wfObj &&
                          wfObj.states &&
                          wfObj.states.length > 0 &&
                          wfObj.states[0].type === "validate"
                        ) {
                ...
                >
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (result?.node?.name) {
                          ...
                          setShowErrors(false);
                          navigate(
                            `/n/${namespace}/explorer${result.node.path}?tab=1&revision=${result.revision.name}&revtab=0`
                          );
                        } else {
                  ...
                  <Button
                    tooltip={`${
                      showErrors ? "Hide Problems" : "Show Problems"
                    }`}
                    color="terminal"
                    variant="contained"
                    style={{
          ...
          {tabBtn === 2 ? (
            <WorkflowDiagram disabled={true} workflow={oldWf} />
          ) : (
            ""
          )}
          {tabBtn === 3 ? (
        ...
        <div style={{ width: "100%" }}>
          {instances !== null && instances.length === 0 ? (
            <div style={{ paddingLeft: "10px", fontSize: "10pt" }}>
              No instances have been recently executed. Recent instances will
              appear here.
            </div>
          ) : (
            <table className="instances-table" style={{ width: "100%" }}>
              <thead>
                <tr>
                  ...
                  <th className="center-align">Revision</th>
                  <th className="center-align">
  ...
  return (
    <>
      <div className="gap">
        <FlexBox gap wrap>
          <FlexBox
            style={{ minWidth: "370px", width: "60%", maxHeight: "342px" }}
          >
            <ContentPanel
              style={{ width: "100%", minWidth: "300px", overflowY: "auto" }}
            >
              <ContentPanelTitle>
                <ContentPanelTitleIcon>
                  <VscVmRunning />
...
function SuccessFailureGraph(props) {
  ...
  if (err !== "") {
    return (
      ...
      >
        <div className="error-message-metrics">
          {err.replace("get failed metrics:", "")}
        </div>
      </FlexBox>
    );
  }
          ...
          >
            <div
              style={{
                height: "8px",
                width: "8px",
                background: "linear-gradient(180deg, #1FEAC5 0%, #25B49A 100%)",
              }}
            />
            Success
          </FlexBox>
          ...
          >
            <div
              style={{
                height: "8px",
                width: "8px",
                background: "linear-gradient(180deg, #F3537E 0%, #DE184D 100%)",
              }}
            />
            Failure
          </FlexBox>
      ...
      <FlexBox col gap style={{ justifyContent: "center" }}>
        ...
        >
          {routes[0] ? (
            <FlexBox col>
              <span>
                {routes.length === 2 ? `${routes[0].weight}%` : `100%`}
              </span>
            </FlexBox>
          ) : (
            ""
...
function WorkflowServices(props) {
  ...
  const [apiKey] = useApiKey();
  const { data, err, deleteWorkflowService } = useWorkflowServices(
    Config.url,
    true,
    namespace,
    filepath.substring(1),
      ...
      <FlexBox col gap>
        ...
        {data.map((obj) => (
          <Service
            ...
            allowRedeploy={true}
            dontDelete={true}
            url={`/n/${namespace}/explorer/${filepath.substring(1)}?function=${
              obj.info.name
        ...
        <FlexBox gap>
                ...
                >
                  ...
                  >
                    ...
                    <FlexBox style={{ width: "100%" }}>
                      <input
                        value={logToEvent}
                        onChange={(e) => setLogToEvent(e.target.value)}
                        type="text"
                        placeholder="Enter the 'event' type to send logs to"
                      />
                    </FlexBox>
                    <div
                      style={{
          ...
          >
            ...
            <ContentPanel style={{ width: "100%" }}>
              ...
              <ContentPanelBody>
                <WorkflowAttributes
                  attributes={workflowData.node.attributes}
                  deleteAttributes={deleteAttributes}
                  addAttributes={addAttributes}
                />
              </ContentPanelBody>
