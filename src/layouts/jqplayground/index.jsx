...
export default function JQPlayground() {
  const [filter, setFilter] = useState(
    localStorage.getItem("jqFilter") ? localStorage.getItem("jqFilter") : "."
  );
  const [input, setInput] = useState(
    localStorage.getItem("jqInput")
      ? localStorage.getItem("jqInput")
  ...
  const { data, err, executeJQ, cheatSheet } = useJQPlayground(
    Config.url,
    apiKey
  );
      ...
      <ContentPanel style={{ width: "100%" }}>
        ...
        <ContentPanelBody>
          <FlexBox col gap style={{ fontSize: "10pt" }}>
            ...
            <ul>
              <li>
                <span style={{ fontWeight: "bold" }}>Filter</span> - This is the
                jq command that will be used to transform your JSON input
              </li>
              <li>
                <span style={{ fontWeight: "bold" }}>JSON</span> - This is the
                JSON input that will be transformed
              </li>
            </ul>
            <div>The transformed JSON is shown in the Result output field.</div>
            <div>
              For information on the JQ syntax, please refer to the offical JQ
              manual online.
            </div>
            <div>
      ...
      <ContentPanel style={{ width: "100%" }}>
        ...
        <FlexBox>
          {error ? (
            ...
            >
              <div>
                <span>error executing JQ command:</span>
                {error.replace("execute jq: error executing JQ command:", "")}
              </div>
            </Alert>
          ) : null}
        </FlexBox>
