...
export function InstancesTable(props) {
  ...
  const queryFilters = useMemo(() => {
    ...
    if (filterState !== "") {
      newFilters.push(
        `filter.field=STATUS&filter.type=MATCH&filter.val=${filterState}`
      );
    }

    if (filterInvoker !== "") {
      newFilters.push(
        `filter.field=TRIGGER&filter.type=CONTAINS&filter.val=${filterInvoker}`
      );
    }
  ...
  const { data, err, pageInfo } = useInstances(
    Config.url,
    true,
    namespace,
    apiKey,
      ...
      {hideTitle ? null : (
        ...
        >
          ...
          <FlexBox col gap>
            ...
            <select
              value={filterState}
              style={{ color: `${filterState === "" ? "gray" : "#082032"}` }}
              onChange={(e) => {
          ...
          <FlexBox col gap>
            ...
            <select
              value={filterInvoker}
              style={{ color: `${filterInvoker === "" ? "gray" : "#082032"}` }}
              onChange={(e) => {
        ...
        <ContentPanelBody style={{ ...bodyStyle }}>
          {data !== null && data.length === 0 ? (
            <div style={{ paddingLeft: "10px", fontSize: "10pt" }}>{`${
              placeholder
                ? placeholder
                : "No instances have been recently executed. Recent instances will appear here."
            }`}</div>
          ) : (
      ...
      {mini ? null : (
        <td
          ...
          style={{
            ...
            textOverflow: "ellipsis",
            overflow: "hidden",
            color: revStr !== undefined ? "" : "var(--theme-dark-gray-text)",
          }}
          className="center-align"
        >
