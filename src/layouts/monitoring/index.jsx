...
function MonitoringPage(props) {
    ...
    if (data !== null) {
      if (clipData === null) {
        ...
        for (let i = 0; i < data.length; i++) {
          cd += `[${dayjs.utc(data[i].t).local().format("HH:mm:ss.SSS")}] ${
            data[i].msg
          }\n`;
        }
        setClipData(cd);
  ...
  return (
    <Loader load={load} timer={3000}>
      <FlexBox
        className="wrap gap"
        style={{ paddingRight: "8px", height: "100%" }}
      >
            ...
            return (
              <tr className="instance-row" key={obj.id}>
                ...
                >
                  <Link
                    style={{ color: "#2396d8" }}
                    to={`/n/${namespace}/explorer/${wf}?tab=1&revision=${revision}&revtab=0`}
                  >
                    {obj.as}
                  </Link>
                </td>
                ...
                >
                  <Link
                    style={{ color: "#2396d8" }}
                    to={`/n/${namespace}/instances/${obj.id}`}
                  >
                    {obj.id.split("-")[0]}
                  </Link>
                </td>
