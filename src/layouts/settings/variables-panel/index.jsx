            ...
            actionButtons={[
              {
                ...
                onClick: async () => {
                  if (document.getElementById("file-picker")) {
                    ...
                    if (keyValue.trim() === "") {
                      throw new Error(
                        "Variable key name needs to be provided."
                      );
                    }
                    if (!file) {
                      throw new Error("Variable file needs to be provided.");
                    }
                    await setNamespaceVariable(
                    ...
                    if (mimeType === "") {
                      throw new Error(
                        "Variable mimetype needs to be provided."
                      );
                    }
                    await setNamespaceVariable(
                      encodeURIComponent(keyValue),
        ...
        >
          <div style={{ width: "100%", paddingRight: "12px", display: "flex" }}>
            <input
              ...
              autoFocus
              placeholder="Enter variable key name"
            />
          </div>
          <div style={{ width: "100%", paddingRight: "12px", display: "flex" }}>
            <select
...
function Variable(props) {
  ...
  return (
    <tr className="body-row" key={`var-${obj.name}${obj.size}`}>
      ...
      <td style={{ width: "80px", maxWidth: "80px", textAlign: "center" }}>
        {fileSize(obj.size)}
      </td>
          ...
          <FlexBox style={{ gap: "2px", justifyContent: "flex-end" }}>
            {!downloading ? (
              <VariablesDownloadButton
                varURL={`${Config.url}namespaces/${namespace}/vars/${obj.name}`}
                varName={`${obj.name}}`}
                onClick={async () => {
                  setDownloading(true);

                  const variableData = await getNamespaceVariableBlob(obj.name);
                  const extension = MimeTypeFileExtension(
                    variableData.contentType
                  );
                  saveAs(
                    variableData.data,
                    obj.name + `${extension ? `.${extension}` : ""}`
                  );
            ...
            <Modal
              ...
              requiredFields={[{ tip: "file is required", value: file }]}
            >
              <FlexBox col gap>
                <VariableFilePicker
                  setMimeType={setType}
...
function VariablesUploadButton() {
  return (
    <div
      className="secrets-delete-btn grey-text auto-margin"
