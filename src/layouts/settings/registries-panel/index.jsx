...
function RegistriesPanel(props) {
  ...
  const { data, getRegistries, createRegistry, deleteRegistry } = useRegistries(
    Config.url,
    namespace,
    apiKey
  );
    ...
    <ContentPanel style={{ height: "100%", minHeight: "180px", width: "100%" }}>
      ...
      <ContentPanelBody className="secrets-panel">
        <FlexBox col gap>
          ...
          <div>
            <Alert severity="info">
              Once a registry is removed, it can never be restored.
            </Alert>
          </div>
        </FlexBox>
      </ContentPanelBody>
...
export function AddRegistryPanel(props) {
  ...
  return (
    <FlexBox col gap style={{ fontSize: "12px" }}>
      ...
      {err ? (
        <FlexBox>
          <Alert severity="error" variant="filled" grow>
            {err}
          </Alert>
        </FlexBox>
      ) : null}
