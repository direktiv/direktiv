...
export const mirrorSettingInfoMetaInfo = {
  url: {
    ...
    info: `URL to repository. If authentication method is SSH Key a git url must be used e.g. "git@github.com:direktiv/apps-svc.git". All other authentication methods must use HTTP/S urls`,
  },
  ref: {
    plainName: "Ref",
    required: true,
  ...
  passphrase: {
    ...
    info: `Passphrase to decrypt keys. (Optional)`,
  },
  token: {
    plainName: "Token",
    required: true,
    placeholder: `Enter personal access token`,
    info: `Personal access token to used for authentication`,
  },
  publicKey: {
    plainName: "Public Key",
    required: true,
    placeholder: `Enter Public Key`,
    info: `Public SSH Key used for authenticating with repository.`,
  },
  privateKey: {
    plainName: "Private Key",
    required: true,
    placeholder: `Enter Private Key`,
    info: `Private SSH Key used for authenticating with repository.`,
  },
};
...
export default function MirrorInfoPanel(props) {
  ...
  const [mirrorSettingsValidateMsg, setMirrorSettingsValidateMsg] =
    useState("");
  ...
  useEffect(() => {
    ...
    if (
      !infoChangesTrackerRef?.current ||
      !typeof infoChangesTrackerRef?.current === "object"
    ) {
      return;
    }
    ...
    <ContentPanel id="panel-mirror-info" style={{ ...style }}>
      <ContentPanelTitle>
        ...
        <FlexBox gap style={{ alignItems: "center" }}>
          <span className="hide-600">Mirror</span> Info
          <FlexBox
            style={{
              flex: "auto",
              ...
              <div>
                      ...
                      onClick: async () => {
                        const newSettings = {
                          ...
                          passphrase: infoChangesTracker.passphrase
                            ? infoPassphrase
                            : "-",
                          publicKey: infoChangesTracker.publicKey
                            ? infoPublicKey
                            : "-",
                          privateKey: infoChangesTracker.privateKey
                            ? infoPrivateKey
                            : "-",
                        };
                ...
                >
                  <FlexBox col gap style={{ height: "fit-content" }}>
                    ...
                    {infoChangesTracker.authMethod ? (
                      <FlexBox
                        className="col center info-update-label"
                        style={{ textAlign: "center" }}
                      >
                        Warning:
                        {mirrorAuthMethod === "none"
                          ? ` Changing authentication method to "None" will remove Passphrase, Public Key and Private Key from settings.`
                          : ""}
                        {mirrorAuthMethod === "token"
                          ? ` Changing authentication method to "Access Token" will remove Public Key and Private Key from settings.`
                          : ""}
                        {mirrorAuthMethod === "ssh"
            ...
            <FlexBox col gap="sm" style={{ paddingRight: "10px" }}>
              <FlexBox row style={{ justifyContent: "space-between" }}>
                <FlexBox row gap="sm" style={{ justifyContent: "flex-start" }}>
                  <span
                    className={`input-title ${
                      infoChangesTracker.passphrase ? "edited" : ""
                    }`}
                  >
                    Token
                  </span>
              ...
              <TextareaAutosize
                ...
                minRows={
                  infoChangesTracker.passphrase || info?.passphrase === ""
                    ? 2
                    : 1
                }
                maxRows={5}
              ...
              <FlexBox col gap="sm" style={{ paddingRight: "10px" }}>
                <FlexBox row style={{ justifyContent: "space-between" }}>
                  ...
                  >
                    ...
                    <HelpIcon
                      msg={mirrorSettingInfoMetaInfo["passphrase"].info}
                    />
                    <HideShowButton
                      show={showPassphrase}
              ...
              <FlexBox col gap="sm" style={{ paddingRight: "10px" }}>
                <FlexBox row style={{ justifyContent: "space-between" }}>
                  ...
                  >
                    <span
                      className={`input-title ${
                        infoChangesTracker.publicKey ? "edited" : ""
                      }`}
                    >
                      Public Key
                    </span>
                    <HelpIcon
                      msg={mirrorSettingInfoMetaInfo["publicKey"].info}
                    />
                  </FlexBox>
                  <FlexBox
                    row
                ...
                <TextareaAutosize
                  ...
                  minRows={
                    infoChangesTracker.publicKey || info?.publicKey === ""
                      ? 2
                      : 1
                  }
                  maxRows={5}
              ...
              <FlexBox col gap="sm" style={{ paddingRight: "10px" }}>
                <FlexBox row style={{ justifyContent: "space-between" }}>
                  ...
                  >
                    <span
                      className={`input-title ${
                        infoChangesTracker.privateKey ? "edited" : ""
                      }`}
                    >
                      Private Key
                    </span>
                    <HelpIcon
                      msg={mirrorSettingInfoMetaInfo["privateKey"].info}
                    />
                  </FlexBox>
                  <FlexBox
                    row
                ...
                <TextareaAutosize
                  ...
                  minRows={
                    infoChangesTracker.privateKey || info?.privateKey === ""
                      ? 2
                      : 1
                  }
                  maxRows={5}
