...
async function executeWF() {
  ...
  const { result, waitForNextUpdate } = renderHook(() =>
    useWorkflow(Config.url, true, Config.namespace, "test-workflow-services")
  );
    ...
    it("fetch service details (fn, revisions, traffic)", async () => {
      const { result, waitForNextUpdate } = renderHook(() =>
        useWorkflowService(
          ...
          Config.namespace,
          "test-workflow-services",
          process.env.WORKFLOW_SERVICE_NAME,
          process.env.WORKFLOW_REVISION
        )
      );
      await waitForNextUpdate();
    ...
    describe("useWorkflowServiceRevision", () => {
      it("get revision details and pods", async () => {
        const { result, waitForNextUpdate } = renderHook(() =>
          useWorkflowServiceRevision(
            ...
            process.env.WORKFLOW_SERVICE_NAME,
            process.env.WORKFLOW_REVISION,
            process.env.WORKFLOW_REV
          )
        );
        await waitForNextUpdate();
        await waitForNextUpdate();
        await waitForNextUpdate();
        expect(result.current.revisionDetails.image).toBe(
          "direktiv/request:v1"
        );
      ...
      describe("usePodLogs", () => {
        it("get pods logs", async () => {
          const { result, waitForNextUpdate } = renderHook(() =>
            usePodLogs(Config.url, process.env.WF_POD_ID)
          );
          await waitForNextUpdate();
...
describe("useNamespaceServices", () => {
  it("fetch namespace services", async () => {
    const { result, waitForNextUpdate } = renderHook(() =>
      useNamespaceServices(Config.url, false, Config.namespace)
    );
    await waitForNextUpdate();
    expect(result.current.data.config.maxscale).toBe(3);
  });
  it("stream namespace services, create a service then delete a service", async () => {
    ...
    await act(async () => {
      await result.current.createNamespaceService(
        "testglobalservice",
        "direktiv/request:v1",
        0,
        0,
  ...
  describe("useNamespaceService", () => {
    ...
    it("create a new revision, set traffic to 50/50 then delete the first revision", async () => {
      ...
      await act(async () => {
        await result.current.createNamespaceServiceRevision(
          "direktiv/request:v1",
          0,
          0,
          "",
      ...
      await act(async () => {
        await result.current.setNamespaceServiceRevisionTraffic(
          result.current.revisions[0].name,
          50,
          result.current.revisions[1].name,
          50
        );
      });
    ...
    describe("usePodLogs", () => {
      it("get pods logs", async () => {
        const { result, waitForNextUpdate } = renderHook(() =>
          usePodLogs(Config.url, process.env.NS_POD_ID)
        );
        await waitForNextUpdate();
        expect(result.current.data.data).toBe("Starting server.\n");
      });
...
describe("useGlobalServices", () => {
  it("fetch global services", async () => {
    const { result, waitForNextUpdate } = renderHook(() =>
      useGlobalServices(Config.url, false)
    );
    await waitForNextUpdate();
    expect(result.current.data.config.maxscale).toBe(3);
  });
  ...
  describe("useGlobalService", () => {
    it("fetch service details (fn, revisions, traffic)", async () => {
      const { result, waitForNextUpdate } = renderHook(() =>
        useGlobalService(Config.url, "service-revision-test-global")
      );
      await waitForNextUpdate();
      await waitForNextUpdate();
      expect(result.current.fn.info.name).toBe("service-revision-test-global");
    ...
    it("create a new revision, set traffic to 50/50 then delete the first revision", async () => {
      ...
      await act(async () => {
        await result.current.createGlobalServiceRevision(
          "direktiv/request:v1",
          0,
          0,
          "",
      ...
      // set traffic to 50/50
      await act(async () => {
        await result.current.setGlobalServiceRevisionTraffic(
          result.current.revisions[0].name,
          50,
          result.current.revisions[1].name,
          50
    ...
    describe("usePodLogs", () => {
      it("get pods logs", async () => {
        const { result, waitForNextUpdate } = renderHook(() =>
          usePodLogs(Config.url, process.env.POD_ID)
        );
        await waitForNextUpdate();
        expect(result.current.data.data).toBe("Starting server.\n");
      });
