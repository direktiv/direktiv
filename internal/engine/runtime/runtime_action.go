...
func (rt *Runtime) service(c map[string]any) sobek.Value {
	...
	data, err := rt.callAction(sd, payload, int(retries), endDuration)
	if err != nil {
...
func (rt *Runtime) action(c map[string]any) sobek.Value {
	...
	actionFunc := func(payload any, timeout string) sobek.Value {
		...
		data, err := rt.callAction(sd, payload, config.Retries, endDuration)
		if err != nil {
			panic(rt.vm.ToValue(err))
		}
...
func (rt *Runtime) callAction(sd *core.ServiceFileData, payload any, retries int, timeout time.Duration) (any, error) {
	if rt.onAction != nil {
	...
	// ping service
	_, err := callRetryable(rt.tracingPack.ctx, svcUrl+"/up", http.MethodGet, []byte(""), 29, 10*time.Second)
	if err != nil {
	...
	outData, err := callRetryable(rt.tracingPack.ctx, svcUrl, http.MethodPost, data, retries, timeout)
	if err != nil {
...
func callRetryable(ctx context.Context, url, method string, payload []byte, retries int, timeout time.Duration) ([]byte, error) {
