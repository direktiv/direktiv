...
func (flow *flow) SetInstanceAnnotation(ctx context.Context, req *grpc.SetInstanceAnnotationRequest) (*grpc.SetInstanceAnnotationResponse, error) {
	...
	if newVar {
		flow.logger.Infof(time.Now(), cached.Instance.ID, cached.GetAttributes("instance"), "Created instance annotation '%s'.", key)
	} else {
		flow.logger.Infof(time.Now(), cached.Instance.ID, cached.GetAttributes("instance"), "Updated instance annotation '%s'.", key)
	}
...
func (flow *flow) DeleteInstanceAnnotation(ctx context.Context, req *grpc.DeleteInstanceAnnotationRequest) (*emptypb.Empty, error) {
	...
	flow.logger.Errorf(time.Now(), cached.Instance.ID, cached.GetAttributes("instance"), "Deleted instance annotation '%s'.", annotation.Name)
	flow.pubsub.NotifyInstanceAnnotations(cached.Instance)
...
func (flow *flow) RenameInstanceAnnotation(ctx context.Context, req *grpc.RenameInstanceAnnotationRequest) (*grpc.RenameInstanceAnnotationResponse, error) {
	...
	flow.logger.Infof(time.Now(), cached.Instance.ID, cached.GetAttributes("instance"), "Renamed instance annotation from '%s' to '%s'.", req.GetOld(), req.GetNew())
	flow.pubsub.NotifyInstanceAnnotations(cached.Instance)
