...
func (flow *flow) SetWorkflowAnnotation(ctx context.Context, req *grpc.SetWorkflowAnnotationRequest) (*grpc.SetWorkflowAnnotationResponse, error) {
	...
	if newVar {
		flow.logger.Infof(time.Now(), cached.Workflow.ID, cached.GetAttributes("workflow"), "Created workflow annotation '%s'.", key)
	} else {
		flow.logger.Infof(time.Now(), cached.Workflow.ID, cached.GetAttributes("workflow"), "Updated workflow annotation '%s'.", key)
	}
...
func (flow *flow) DeleteWorkflowAnnotation(ctx context.Context, req *grpc.DeleteWorkflowAnnotationRequest) (*emptypb.Empty, error) {
	...
	flow.logger.Infof(time.Now(), cached.Workflow.ID, cached.GetAttributes("workflow"), "Deleted workflow annotation '%s'.", annotation.Name)
	flow.pubsub.NotifyWorkflowAnnotations(cached.Workflow)
...
func (flow *flow) RenameWorkflowAnnotation(ctx context.Context, req *grpc.RenameWorkflowAnnotationRequest) (*grpc.RenameWorkflowAnnotationResponse, error) {
	...
	flow.logger.Infof(time.Now(), cached.Workflow.ID, cached.GetAttributes("workflow"), "Renamed workflow annotation from '%s' to '%s'.", req.GetOld(), req.GetNew())
	flow.pubsub.NotifyWorkflowAnnotations(cached.Workflow)
