...
func (events *events) ReplayCloudevent(ctx context.Context, cached *database.CacheData, cevent *ent.CloudEvents) error {
	...
	events.logger.Infof(ctx, cached.Namespace.ID, cached.GetAttributes("namespace"), "Replaying event: %s (%s / %s)", event.ID(), event.Type(), event.Source())
...
func (events *events) BroadcastCloudevent(ctx context.Context, cached *database.CacheData, event *cloudevents.Event, timer int64) error {
	events.logger.Infof(ctx, cached.Namespace.ID, cached.GetAttributes("namespace"), "Event received: %s (%s / %s)", event.ID(), event.Type(), event.Source())
...
func (flow *flow) ApplyCloudEventFilter(ctx context.Context, in *grpc.ApplyCloudEventFilterRequest) (*emptypb.Empty, error) {
	...
	err = vm.Set("nslog", func(txt interface{}) {
		flow.logger.Infof(ctx, cached.Namespace.ID, cached.GetAttributes("namespace"), fmt.Sprintf("%v", txt))
	})
	if err != nil {
		return resp, fmt.Errorf("failed to initialize js runtime: %w", err)
	}
	...
	if err != nil {
		flow.logger.Errorf(ctx, cached.Namespace.ID, cached.GetAttributes("namespace"), "CloudEvent filter '%s' produced an error (1): %v", filterName, err)
		return resp, err
	}

	f, ok := goja.AssertFunction(vm.Get("filter"))
	if !ok {
		flow.logger.Errorf(ctx, cached.Namespace.ID, cached.GetAttributes("namespace"), "cloudEvent filter '%s' error: %v", filterName, err)
		return resp, err
	}

	newEventMap, err := f(goja.Undefined())
	if err != nil {
		flow.logger.Errorf(ctx, cached.Namespace.ID, cached.GetAttributes("namespace"), "CloudEvent filter '%s' produced an error (2): %v", filterName, err)
		return resp, err
	}
	...
	if err != nil {
		flow.logger.Errorf(ctx, cached.Namespace.ID, cached.GetAttributes("namespace"), "CloudEvent filter '%s' produced an error (3): %v", filterName, err)
		return resp, err
	}
