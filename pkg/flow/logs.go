...
func (srv *server) logToInstanceRaw(ctx context.Context, t time.Time, in *ent.Instance, tags map[string]string, msg string) {
...
func (srv *server) storeLogMsg(l *logMessage) error {
	...
	// 	tags += "[" + k + ":" + v + "]"
	// }
	// lc := tx.LogMsg.Create().SetMsg(tags + l.msg).SetT(l.t)
