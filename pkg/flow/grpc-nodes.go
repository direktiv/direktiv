...
func (flow *flow) Directory(ctx context.Context, req *grpc.DirectoryRequest) (*grpc.DirectoryResponse, error) {
	...
	err = flow.runSqlTx(ctx, func(tx *sqlTx) error {
		...
		node, err = tx.FileStore().ForRootNamespace(ns.Name).GetFile(ctx, req.GetPath())
		if err != nil {
			return err
		}
		files, err = tx.FileStore().ForRootNamespace(ns.Name).ReadDirectory(ctx, req.GetPath())
		if err != nil {
			return err
		}
		return nil
...
func (flow *flow) CreateDirectory(ctx context.Context, req *grpc.CreateDirectoryRequest) (*grpc.CreateDirectoryResponse, error) {
	...
	file, _, err = tx.FileStore().ForRootNamespace(ns.Name).CreateFile(ctx, req.GetPath(), filestore.FileTypeDirectory, "", nil)
	if err != nil {
		return nil, err
	}
...
func (flow *flow) DeleteNode(ctx context.Context, req *grpc.DeleteNodeRequest) (*emptypb.Empty, error) {
	...
	if file.Typ == filestore.FileTypeDirectory {
		isEmptyDir, err := tx.FileStore().ForRootNamespace(ns.Name).IsEmptyDirectory(ctx, req.GetPath())
		if err != nil {
			return nil, err
		}
...
func (flow *flow) RenameNode(ctx context.Context, req *grpc.RenameNodeRequest) (*grpc.RenameNodeResponse, error) {
	...
	file, err := tx.FileStore().ForRootNamespace(ns.Name).GetFile(ctx, req.GetOld())
	if err != nil {
		return nil, err
	}
