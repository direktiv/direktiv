...
func (flow *flow) createDirectory(ctx context.Context, args *createDirectoryArgs) (*database.Inode, error) {
	...
	flow.logger.LogToNamespace(ctx, time.Now(), args.pcached, "Created directory '%s'.", args.path)
...
func (flow *flow) deleteNode(ctx context.Context, args *deleteNodeArgs) error {
	...
	flow.logger.LogToNamespace(ctx, time.Now(), args.cached, "Deleted %s '%s'.", args.cached.Inode().Type, args.cached.Path())
...
func (flow *flow) RenameNode(ctx context.Context, req *grpc.RenameNodeRequest) (*grpc.RenameNodeResponse, error) {
	...
	flow.logger.LogToNamespace(ctx, time.Now(), cached, "Renamed %s from '%s' to '%s'.", cached.Inode().Type, req.GetOld(), req.GetNew())
