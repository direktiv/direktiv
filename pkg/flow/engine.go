...
func (engine *engine) CrashInstance(ctx context.Context, im *instanceMemory, err error) {
	...
	if errors.As(err, &cerr) {
		...
		engine.logToInstance(ctx, time.Now(), im, "Instance failed with error '%s': %s", cerr.Code, err.Error())
	} else if errors.As(err, &uerr) && uerr.Code != "" {
		engine.sugar.Errorf("Instance failed with uncatchable error '%s': %v", uerr.Code, err)
		engine.logToInstance(ctx, time.Now(), im, "Instance failed with uncatchable error '%s': %s", uerr.Code, err.Error())
	} else {
		engine.sugar.Errorf("Instance failed with uncatchable error: %v", err)
		engine.logToInstance(ctx, time.Now(), im, "Instance failed with uncatchable error: %s", err.Error())
	}
...
failure:
	...
	if errors.As(err, &cerr) {
		...
		for i, catch := range im.logic.ErrorDefinitions() {
			...
			if regErr != nil {
				engine.logToInstance(ctx, t, im, "Error catching regex failed to compile: %v", regErr)
			}

			if matched {

				engine.logToInstance(ctx, t, im, "State failed with error '%s': %s", cerr.Code, cerr.Message)
				engine.logToInstance(ctx, t, im, "Error caught by error definition %d: %s", i, catch.Error)
...
func (engine *engine) transformState(ctx context.Context, im *instanceMemory, transition *states.Transition) error {
	...
	engine.logToInstance(ctx, time.Now(), im, "Transforming state data.")
...
func (engine *engine) transitionState(ctx context.Context, im *instanceMemory, transition *states.Transition, errCode string) {
	...
	if transition.NextState != "" {
		...
		engine.logToInstance(ctx, time.Now(), im, "Transitioning to next state: %s (%d).", transition.NextState, im.Step()+1)
	...
	if im.ErrorCode() != "" {
		...
		engine.logToInstance(ctx, time.Now(), im, "Workflow failed with error '%s': %s", im.ErrorCode(), im.in.ErrorMessage)
	}
	...
	engine.logToInstance(ctx, time.Now(), im, "Workflow completed.")
