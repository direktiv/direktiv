...
func (logic *actionLogic) Deadline(ctx context.Context) time.Time {
	...
	if err != nil {
		if logic.Timeout != "" {
			logic.Log(ctx, logengine.Error, "failed to parse timeout: %v for %s", err, logic.label())
...
func (logic *actionLogic) scheduleAction(ctx context.Context, attempt int) error {
	...
	logic.Log(ctx, logengine.Info, "Sleeping until function %s returns (%s).", logic.label(), child.ID)
...
func (logic *actionLogic) scheduleRetryAction(ctx context.Context, retry *actionRetryInfo) error {
	logic.Log(ctx, logengine.Info, "Retrying %s...", logic.label())
...
func (logic *actionLogic) processActionResults(ctx context.Context, children []*ChildInfo, results *actionResultPayload) (*Transition, error) {
	...
	logic.Log(ctx, logengine.Info, "child %s returned, id: %s.", logic.label(), id)

	if results.ErrorCode != "" {
		logic.Log(ctx, logengine.Error, "Action %s raised catchable error '%s': %s.", logic.label(), results.ErrorCode, results.ErrorMessage)
		...
		logic.Log(ctx, logengine.Info, "Scheduling retry attempt in: %v for action %s", d, logic.label())

		return nil, scheduleRetry(ctx, logic.Instance, children, 0, d)
	}

	if results.ErrorMessage != "" {
		logic.Log(ctx, logengine.Error, "Action %s crashed due to an internal error: %v", logic.label(), results.ErrorMessage)
