...
func (logic *parallelLogic) Deadline(ctx context.Context) time.Time {
	...
	if err != nil {
		if logic.Timeout != "" {
			logic.Log(ctx, logengine.Error, "failed to parse timeout: %v", err)
		}
...
func (logic *parallelLogic) scheduleFirstActions(ctx context.Context) error {
	...
	logic.Log(ctx, logengine.Info, "Sleeping until children return.")
...
func (logic *parallelLogic) scheduleRetryAction(ctx context.Context, retry *actionRetryInfo) error {
	logic.Log(ctx, logengine.Info, "Retrying...")
...
func (logic *parallelLogic) processActionResults(ctx context.Context, children []*ChildInfo, results *actionResultPayload) (*Transition, error) {
	...
	logic.Log(ctx, logengine.Info, "Child '%s' returned.", id)

	if results.ErrorCode != "" {
		logic.Log(ctx, logengine.Error, "Action raised catchable error '%s': %s.", results.ErrorCode, results.ErrorMessage)
		...
		logic.Log(ctx, logengine.Info, "Scheduling retry attempt in: %v.", d)

		return nil, scheduleRetry(ctx, logic.Instance, children, idx, d)
	}

	if results.ErrorMessage != "" {
		logic.Log(ctx, logengine.Error, "Action crashed due to an internal error: %v", results.ErrorMessage)
	...
	case model.BranchModeAnd:
		...
		logic.Log(ctx, logengine.Info, "Action returned. (%d/%d)", completed, len(children))
	...
	case model.BranchModeOr:
		...
		if results.ErrorCode != "" {
			...
			logic.Log(ctx, logengine.Error, "Branch %d crashed due to an internal error: %s", idx, results.ErrorMessage)
