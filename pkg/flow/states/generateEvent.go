...
func (logic *generateEventLogic) Run(ctx context.Context, wakedata []byte) (*Transition, error) {
	...
	if s, ok := x.(string); ok && ctype != "" && ctype != "application/json" {
		...
		if err != nil {
			logic.Log(ctx, logengine.Error, "Unable to set event data: %v", err)
		}
	}
	...
	for k, v := range logic.Event.Context {
		...
		logic.Log(ctx, logengine.Debug, "Adding context %v: %v", k, x)

		err = event.Context.SetExtension(k, x)
		if err != nil {
			logic.Log(ctx, logengine.Error, "Unable to set event extension: %v", err)
		}
	}

	logic.Log(ctx, logengine.Info, "Broadcasting event type:%s/source:%s to this namespace.", event.Type(), event.Source())
