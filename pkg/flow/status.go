...
func (engine *engine) SetInstanceFailed(ctx context.Context, im *instanceMemory, err error) {
	...
	if errors.As(err, &uerr) {
		...
		telemetry.LogInstanceError(ctx, "workflow instance encountered an internal error",
			fmt.Errorf("internal error: %w", ierr))
		status = instancestore.InstanceStatusCrashed
		message = "an internal error occurred"
	} else {
		telemetry.LogInstanceError(ctx, "workflow instance failed due to an unhandled error",
