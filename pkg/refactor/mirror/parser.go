...
func NewParser(namespace string, activity uuid.UUID, src Source) (*Parser, error) {
	...
	err = p.parse()
	if err != nil {
		slog.Error("Processing repository in temporary directory", "error", err, "namespace", p.namespace, "track", recipient.Mirror.String()+"."+p.activity.String())
		_ = p.Close()
...
func (p *Parser) loadIgnores() error {
	...
	if errors.Is(err, os.ErrNotExist) {
		slog.Debug("No .direktivignore file detected", "track", recipient.Mirror.String()+"."+p.activity.String())
...
func (p *Parser) scanAndPruneDirektivResourceFile(path string) error {
	...
	if err != nil {
		slog.Error("loading possible Direktiv resource definition", "error", err, "namespace", p.namespace, "track", recipient.Mirror.String()+"."+p.activity.String())
...
func (p *Parser) scanAndPruneAmbiguousDirektivWorkflowFile(path string) error {
	...
	err = wf.Load(data)
	if err != nil {
		slog.Error("Error loading possible Direktiv workflow definition (ambiguous)", "path", path, "error", err, "track", recipient.Mirror.String()+"."+p.activity.String())
...
func (p *Parser) handleFilters(path string, filters *model.Filters) error {
	...
	for idx, filter := range filters.Filters {
		...
		slog.Debug("Filter loaded.", "filer_name", filter.Name, "namespace", p.namespace)
	}
...
func (p *Parser) parseDeprecatedVariableFiles() error {
	...
	for _, fpath := range allFiles {
		...
		if !regex.MatchString(vname) {
			slog.Error("Detected a possible deprecated namespace variable definition with an invalid name", "path", fpath, "track", recipient.Mirror.String()+"."+p.activity.String(), "namespace", p.namespace)
		...
		if strings.HasPrefix(base, prefix) {
			...
			slog.Error("Detected deprecated namespace variable definition", "path", fpath, "track", recipient.Mirror.String()+"."+p.activity.String(), "namespace", p.namespace)
		}
	}
