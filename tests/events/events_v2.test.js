...
describe('Test send events v2 api', () => {
	...
	it(`should send event to namespace`, async () => {
		const sendEventResponse = await request(common.config.getDirektivBaseUrl()).post(`/api/v2/namespaces/${ namespaceName }/events/broadcast`)
	...
	it(`should be regitered`, async () => {
		const eventHistoryResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/history`)
			.send()
	...
	it(`event by id`, async () => {
		const eventHistoryResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/history/123`)
			.send()
		expect(eventHistoryResponse.statusCode).toEqual(200)
	})
})
...
`)
	...
	it(`listener should be regitered`, async () => {
		const eventListenerResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/listeners?limit=100&offset=0`)
			.send()
	...
	it(`listener by id`, async () => {
		const eventListenerResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/listeners/${ tmpid }`)
			.send()
		expect(eventListenerResponse.statusCode).toEqual(200)
