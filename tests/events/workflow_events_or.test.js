...
describe('Test workflow events and', () => {
	...
	it(`should kick off in flow workflow`, async () => {
		...
		instancesResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/instances?limit=10&offset=0&filter.field=AS&filter.type=CONTAINS&filter.val=` + waitWorkflowName)
			.send()
	...
	retry10(`should kick off start event workflow stage 2`, async () => {
		const instancesResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/instances?limit=10&offset=0&filter.field=AS&filter.type=CONTAINS&filter.val=` + startWorkflowName)
			.send()
		expect(instancesResponse.body.meta.total).toEqual(2)
	})
	...
	it(`should timeout flow`, async () => {
		...
		// start workflow
		const runWorkflowResponse = await request(common.config.getDirektivBaseUrl()).post(`/api/v2/namespaces/${ namespaceName }/instances?path=${ waitWorkflowTimeoutName }`)
			.send()
		expect(runWorkflowResponse.statusCode).toEqual(200)
