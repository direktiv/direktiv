...
describe('Test workflow events and', () => {
	...
	it(`should kick off in flow workflow`, async () => {
		...
		await events.sendEventAndList(
			namespaceName,
			basevent('eventtype2', 'eventtype2a', 'world2'),
		)

		// there are two workflows now
		instancesResponse = await request(common.config.getDirektivBaseUrl())
			.get(
				`/api/v2/namespaces/${namespaceName}/instances?limit=10&offset=0&filter.field=AS&filter.type=CONTAINS&filter.val=` +
					waitWorkflowName,
			)
			.send()
	...
	retry10(`should kick off start event workflow stage 2`, async () => {
		const instancesResponse = await request(common.config.getDirektivBaseUrl())
			.get(
				`/api/v2/namespaces/${namespaceName}/instances?limit=10&offset=0&filter.field=AS&filter.type=CONTAINS&filter.val=` +
					startWorkflowName,
			)
	...
	it(`should timeout flow`, async () => {
		...
		const runWorkflowResponse = await request(
			...
			.post(
				`/api/v2/namespaces/${namespaceName}/instances?path=${waitWorkflowTimeoutName}`,
			)
			.send()
		expect(runWorkflowResponse.statusCode).toEqual(200)
		await new Promise((r) => setTimeout(r, 7000))

		const instancesResponse = await events.listInstancesAndFilter(
			namespaceName,
			waitWorkflowTimeoutName,
			'failed',
		)
