...
describe('Test send events', () => {
	...
	it(`fails with duplicate id`, async () => {
		const broadcastResponse = await request(common.config.getDirektivBaseUrl()).post(`/api/v2/namespaces/${ namespaceName }/events/broadcast`)
			.set('Content-Type', 'application/json')
			.send(eventDuplicate)
		const historyResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/history?limit=10&offset=0`)
			.send()
	...
	it(`should send event to namespace with JSON`, async () => {
		const broadcastResponse = await request(common.config.getDirektivBaseUrl()).post(`/api/v2/namespaces/${ namespaceName }/events/broadcast`)
			.set('Content-Type', 'application/json')
			.send(eventWithJSON)
		const workflowEventListResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/history?limit=10&offset=0`)
			.send()
	...
	it(`bad filter value applied on the eventlog`, async () => {
		...
		const historyResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/history?limit=10&offset=0&eventContains=dfda`)
			.send()
	...
	it(`should filter the eventlog by TEXT`, async () => {
		const historyResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/history?limit=10&offset=0&filter.field=TEXT&eventContains=world`)
			.send()
	...
	it(`should filter the eventlog by TYPE`, async () => {
		const historyResponse = await request(common.config.getDirektivBaseUrl()).get(`/api/v2/namespaces/${ namespaceName }/events/history?limit=10&offset=0&filter.field=TYPE&typeContains=testerJSON`)
			.send()
