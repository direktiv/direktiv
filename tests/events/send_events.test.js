...
describe('Test send events', () => {
	...
	it(`fails with duplicate id`, async () => {
		const broadcastResponse = await request(common.config.getDirektivBaseUrl())
			.post(`/api/v2/namespaces/${namespaceName}/events/broadcast`)
			.set('Content-Type', 'application/json')
			.send(eventDuplicate)
		const historyResponse = await request(common.config.getDirektivBaseUrl())
			.get(
				`/api/v2/namespaces/${namespaceName}/events/history?limit=10&offset=0`,
			)
			.send()
	...
	it(`should send event to namespace with JSON`, async () => {
		const broadcastResponse = await request(common.config.getDirektivBaseUrl())
			...
			.set('Content-Type', 'application/json')
			.send(eventWithJSON)
		const workflowEventListResponse = await request(
			common.config.getDirektivBaseUrl(),
		)
			.get(
				`/api/v2/namespaces/${namespaceName}/events/history?limit=10&offset=0`,
	...
	it(`should list events`, async () => {
		...
		expect(
			historyResponse.body.data.find(
				(item) => item.event.type === 'testerDuplicate',
			),
		).not.toBeFalsy()
		expect(
			historyResponse.body.data.find((item) => item.event.type === 'testerXML'),
		).not.toBeFalsy()
		expect(
			historyResponse.body.data.find(
				(item) => item.event.type === 'testerJSON',
			),
		).not.toBeFalsy()
	})
	...
	it(`bad filter value applied on the eventlog`, async () => {
		...
		const historyResponse = await request(common.config.getDirektivBaseUrl())
			.get(
				`/api/v2/namespaces/${namespaceName}/events/history?limit=10&offset=0&eventContains=dfda`,
			)
			.send()
		...
		expect(historyResponse.body.data).toEqual(expect.arrayContaining([]))
	})

	it(`should filter the eventlog by TEXT`, async () => {
		const historyResponse = await request(common.config.getDirektivBaseUrl())
			.get(
				`/api/v2/namespaces/${namespaceName}/events/history?limit=10&offset=0&filter.field=TEXT&eventContains=world`,
			)
			.send()
	...
	it(`should filter the eventlog by TYPE`, async () => {
		const historyResponse = await request(common.config.getDirektivBaseUrl())
			.get(
				`/api/v2/namespaces/${namespaceName}/events/history?limit=10&offset=0&filter.field=TYPE&typeContains=testerJSON`,
			)
			.send()
