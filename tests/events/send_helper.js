...
async function listInstancesAndFilter(ns, wf, status) {
	...
	if (wf) append = '&filter.field=AS&filter.type=CONTAINS&filter.val=' + wf

	let instancesResponse = await request(common.config.getDirektivBaseUrl())
		.get(`/api/v2/namespaces/${ns}/instances?limit=100&offset=0` + append)
		.send()
	...
	if (wf || status)
		for (let i = 0; i < 20; i++) {
			instancesResponse = await request(common.config.getDirektivBaseUrl()).get(
				`/api/v2/namespaces/${ns}/instances?limit=100&offset=0` + append,
			)
			if (status) {
				const idFind = instancesResponse.body.data.find(
					(item) => item.status === status,
				)
				if (idFind) return idFind
	...
	if (instancesResponse) return instancesResponse.body
}
...
async function sendEventAndList(ns, event) {
	...
	const workflowEventResponse = await request(
		...
		.post(`/api/v2/namespaces/${ns}/events/broadcast`)
		.set('Content-Type', 'application/json')
		.send(event)
	expect(workflowEventResponse.statusCode).toEqual(200)

	// wait for it to be registered
	const eventsResponse = await request(common.config.getDirektivBaseUrl())
		.get(`/api/v2/namespaces/${ns}/events/history?limit=100&offset=0`)
		.send()
