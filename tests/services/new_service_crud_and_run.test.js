...
`,
	...
	retry50(`should list all service pods`, async () => {
		...
		let res = await request(common.config.getDirektivBaseUrl()).get(
			`/api/v2/namespaces/${testNamespace}/services/${sID}/pods`,
		)
		expect(res.statusCode).toEqual(200)
		expect(res.body).toMatchObject({
			data: [{ id: expect.stringMatching(`^${sID}(_|-)`) }],
		})

		sID = listRes.body.data[1].id
		res = await request(common.config.getDirektivBaseUrl()).get(
			`/api/v2/namespaces/${testNamespace}/services/${sID}/pods`,
		)
		expect(res.statusCode).toEqual(200)
		expect(res.body).toMatchObject({
	...
	it(`should rebuild all services`, async () => {
		...
		let res = await request(common.config.getDirektivBaseUrl())
			.post(
				`/api/v2/namespaces/${testNamespace}/services/${sID}/actions/rebuild`,
			)
			.send()
		expect(res.statusCode).toEqual(200)
