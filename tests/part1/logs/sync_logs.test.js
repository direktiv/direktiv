...
describe('Test sync api calls', () => {
	...
	it(`create namespace`, async () => {
		const res = await request(common.config.getDirektivBaseUrl())
			...
			.send(
				`{"name":"${namespace}","mirror":{"authType":"public","gitRef":"main","url":"https://github.com/direktiv/dproxy","insecure":false}}`,
			)
		expect(res.statusCode).toEqual(200)

		const resSync = await request(common.config.getDirektivBaseUrl()).post(
	...
	retry50(`get sync logs`, async () => {
		...
		const resLog = await request(common.config.getDirektivBaseUrl()).get(
			`/api/v2/namespaces/${namespace}/logs?activity=${res.body.data[0].id}`,
		)
