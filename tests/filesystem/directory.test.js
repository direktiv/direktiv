...
describe('Test basic directory operations', () => {
    ...
    it(`should fail to create a non-root directory because of a missing/invalid 'op' param`, async () => {
        ...
        var createDirectoryResponse2 = await request(common.config.getDirektivHost()).put(`/api/namespaces/${namespaceName}/tree/${subdirName}?op=delete-directory`)
        var createDirectoryResponse3 = await request(common.config.getDirektivHost()).put(`/api/namespaces/${namespaceName}/tree/${subdirName}?opa=create-directory`)
    ...
    it(`should fail to create a non-root directory because of a bad method`, async () => {
        var createDirectoryResponse1 = await request(common.config.getDirektivHost()).post(`/api/namespaces/${namespaceName}/tree/${subdirName}?op=create-directory`)
        var createDirectoryResponse2 = await request(common.config.getDirektivHost()).get(`/api/namespaces/${namespaceName}/tree/${subdirName}?op=create-directory`)
        var createDirectoryResponse3 = await request(common.config.getDirektivHost()).patch(`/api/namespaces/${namespaceName}/tree/${subdirName}?op=create-directory`)
        var createDirectoryResponse4 = await request(common.config.getDirektivHost()).delete(`/api/namespaces/${namespaceName}/tree/${subdirName}?op=create-directory`)
    ...
    it(`should read the sub-directory`, async () => {
        var readNonRootDirResponse = await request(common.config.getDirektivHost()).get(`/api/namespaces/${namespaceName}/tree/${subdirName}`)
    ...
    it(`should fail to delete the empty sub-directory due to a missing op param`, async () => {
        var deleteDirectoryResponse = await request(common.config.getDirektivHost()).delete(`/api/namespaces/${namespaceName}/tree/${subdirName}`)
    ...
    it(`should fail to delete the empty sub-directory due to a bad method`, async () => {
        var deleteDirectoryResponse = await request(common.config.getDirektivHost()).post(`/api/namespaces/${namespaceName}/tree/${subdirName}?op=delete-node`)
        expect(deleteDirectoryResponse.statusCode).toEqual(405)
        expect(deleteDirectoryResponse.body).toMatchObject({})
