...
async function deleteAllNamespaces() {
  const listResponse = await request(config.getDirektivHost()).get(
    `/api/v2/namespaces`
  );
  if (listResponse.statusCode !== 200)
    throw Error(
      `none ok namespaces list statusCode(${listResponse.statusCode})`
    );

  for (const namespace of listResponse.body.data) {
    const response = await request(config.getDirektivHost()).delete(
      `/api/v2/namespaces/${namespace.name}`
    );

    if (response.statusCode !== 200)
      throw Error(
        `none ok namespace(${namespace.name}) delete statusCode(${response.statusCode})`
      );
  }
}
...
async function itShouldCreateFile(
  it,
  expect,
  ns,
  path,
...
async function itShouldUpdateFile(it, expect, ns, path, newPatch) {
  ...
  it(title, async () => {
    ...
    if (newPatch.path !== undefined) want.path = newPatch.path;

    if (newPatch.data !== undefined) want.data = newPatch.data;
...
async function itShouldCheckPathExists(it, expect, ns, path, assertExits) {
  it(`should check if path(${path}) exists(${assertExits})`, async () => {
    const res = await request(common.config.getDirektivHost()).get(
      `/api/v2/namespaces/${ns}/files${path}`
    );

    if (assertExits) expect(res.statusCode).toEqual(200);
...
async function itShouldDeleteFile(it, expect, ns, path) {
  it(`should delete a file ${path}`, async () => {
    const res = await request(common.config.getDirektivHost()).delete(
      `/api/v2/namespaces/${ns}/files${path}`
    );
