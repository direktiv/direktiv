	...
	it(`should list /main.wf.ts workflow history`, async () => {
		...
		expect(history).toEqual([
			...
			},
			{
				...
				output: undefined,
				error:
					'invoke start: ReferenceError: threw is not defined at stateTwo (subflow.wf.ts:7:1(1))',
				sequence: firstSequence++,
			},
			{
				scope: 'main',
				...
				output: undefined,
				error:
					'invoke start: error calling on subflow: subflow did not complete: invoke start: ReferenceError: threw is not defined at stateTwo (subflow.wf.ts:7:1(1)) at github.com/direktiv/direktiv/internal/engine/runtime.(*Runtime).execSubflow-fm (native)',
				sequence: firstSequence++,
			},
		])
	})
