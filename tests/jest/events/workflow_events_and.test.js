...
const startWorkflowTimeoutName = "startandtimeout.yaml"
const startEventTimeoutWorkflow = `
start:
  type: eventsAnd
  state: greet
  lifespan: PT1S
...
describe('Test workflow events and', () => {
    ...
    // it(`should timeout start`, async () => {
        
    //   // timeout workflow
    //   var createWorkflowResponse = await request(common.config.getDirektivHost()).put(`/api/namespaces/${namespaceName}/tree/${startWorkflowTimeoutName}?op=create-workflow`)
    //     .send(startEventTimeoutWorkflow)
    //     expect(createWorkflowResponse.statusCode).toEqual(200)

    //     await events.sendEventAndList(namespaceName, basevent("eventtype5", "eventtype5", "world1"))
    //     // TODO: this starts the workflow but should not because the timeout/lifespan should fail
    //     // await new Promise((r) => setTimeout(r, 3000));
    //     await events.sendEventAndList(namespaceName, basevent("eventtype6", "eventtype6", "world1"))

    //     var instance = await events.listInstancesAndFilter(namespaceName, startWorkflowTimeoutName)  
    //     // expect(instance).toBeFalsy();
    ...
    //   await events.sendEventAndList(namespaceName, basevent("eventtype7", "eventtype7", "world1"))
    //   await new Promise((r) => setTimeout(r, 8000));
    //   await events.sendEventAndList(namespaceName, basevent("eventtype8", "eventtype8", "world1"))

    //   var instance = await events.listInstancesAndFilter(namespaceName, waitWorkflowTimeoutName, "failed")
    //   expect(instance).not.toBeFalsy();
